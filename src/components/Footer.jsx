...
const Footer = () =>(
  <section className={`${styles.flexCenter} ${styles.paddingY} flex-col`}>
    {/* footer logo and heading and link */}
    ...
    <div className='w-full flex justify-between items-center md:flex-row flex-col pt-6 border-t-[1px] border-t-[#3F3E45]'>
      ...
      <div className='flex flex-row md:mt-0 mt-6'>
        {socialMedia.map((social, index) => (
          <img key={social.id} src={social.icon} alt='social' className={`w-[21px] h-[21px] object-contain cursor-pointer first-letter: ${index !== socialMedia.length -1 ? 'mr-6' : 'mr-0'}`} onClick={() => window.open(social.link)} />
        ))}
